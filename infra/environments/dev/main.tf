...
resource "random_id" "frontend_tg" {
  byte_length = 2   # 2 bytes → 4 dígitos hexadecimales
}

resource "aws_lb_target_group" "frontend_tg" {
  name        = "frontend-${var.environment}-tg-${random_id.frontend_tg.hex}"
  port        = 80
  protocol    = "HTTP"
  vpc_id      = aws_vpc.main.id
  target_type = "ip"
