        ...
        function fetchParticipantes() {
            $.get('http://localhost:8000/api/microservice1/participantes', function (response) { 
                ...
                response.data.forEach((item, i) => {
                    ...
                    $.get('http://localhost:8000/api/microservice2/votaciones/resultados/'+item.par_codigo, function (votosData) {
...
function votarPorParticipante(par_codigo) {
    ...
    fetch('http://localhost:8000/api/microservice2/votaciones/voto', {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
...
$('#tablavotaciones').DataTable({
    ...
    'stateSave': true,
    'ajax': {
        "url": "http://localhost:8000/api/microservice2/listavotaciones",
        "type": "GET",
        "dataSrc": "data",
...
function borravoto(codigo) {
    if (confirm("¿Estás seguro de que deseas eliminar este voto? Esta acción no se puede deshacer.")) {
        ...
        fetch("http://localhost:8000/api/microservice2/borravoto", {
            method: "DELETE",
            headers: {
